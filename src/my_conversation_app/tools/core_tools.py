...
from my_conversation_app.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH
...
if TYPE_CHECKING:
    from my_conversation_app.mcp_client import RemoteMcpToolClient
...
def _load_profile_tools(tool_names: list[str], remote_tool_names: set[str]) -> List[type[Tool]]:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"my_conversation_app.tools.{tool_name}"
            try:
