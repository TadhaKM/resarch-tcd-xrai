...
from my_conversation_app.config import HF_AVAILABLE_VOICES, config
from my_conversation_app.console import LocalStream
from my_conversation_app.startup_settings import (
    StartupSettings,
    load_startup_settings_into_runtime,
)
from my_conversation_app.personality_routes import (
    RouteError,
...
def test_backend_startup_failure_is_recorded_without_raising(
    ...
    monkeypatch.setattr("my_conversation_app.console.apply_audio_startup_config", MagicMock())
...
def test_media_warmup_overlaps_audio_startup_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("my_conversation_app.console.has_hf_realtime_target", lambda: True)
    ...
    monkeypatch.setattr("my_conversation_app.console.asyncio.sleep", wait_for_audio_config)
    monkeypatch.setattr("my_conversation_app.console.apply_audio_startup_config", apply_audio_config)
...
async def test_apply_personality_propagates_restart_cancellation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("my_conversation_app.config.set_custom_profile", lambda _profile: None)
    monkeypatch.setattr(
        "my_conversation_app.prompts.get_session_instructions", lambda _instance_path=None: "instructions"
    )
    monkeypatch.setattr("my_conversation_app.prompts.get_session_voice", lambda default: default)
...
def test_local_stream_persist_personality_clears_legacy_startup_env_overrides(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "my_conversation_app.config.set_custom_profile",
...
def test_local_stream_launch_waits_for_missing_hf_target_without_starting_media(
    ...
    monkeypatch.setattr("my_conversation_app.console.time.sleep", MagicMock(side_effect=KeyboardInterrupt))
